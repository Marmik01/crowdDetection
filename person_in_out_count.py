...
def detect(weights=ROOT / 'yolov5s.pt',
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            cv2.putText(im0, 'ENTRY : ' + str(totalDownPerson), (int(width * 0.6), int(height * 0.05)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'EXIT : ' + str(totalUpPerson), (int(width * 0.02), int(height * 0.05)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            if view_img:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'crowdhuman_yolov5m.pt', help='model path(s)')
